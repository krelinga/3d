...
headless EGL PNG rendering, a different toolchain than CI uses). This doc
...
(`python3` for the metrics tooling, `convert` for thumbnail checks) is then
...
`text()` was avoided on the grounds that fonts were not pinned the way the
rest of the toolchain is (issue #8). Measurement shows the premise was half
wrong and the real hazard is somewhere else.

### Fonts are already pinned, because they live in the image

Two sets ship inside the pinned digest, so they are covered by exactly the
same guarantee as `openscad` itself:
...
Nothing extra is needed to pin them. A pin bump can change the set, which is
exactly the visibility the digest exists to give.

Note the two are discovered differently. `fc-list` reports only the DejaVu
families; OpenSCAD's font path does not include its own bundled directory, yet
`font = "Liberation Sans"` resolves anyway because OpenSCAD loads that set
internally. **Availability cannot be established with `fc-list`.**

### Rendering is deterministic

Three renders of the same string produced identical geometry
(`40.8145 x 10.3563 x 2 mm`, `206.743 mm3`, 1688 facets), with and without an
explicit `font=`. The `Fontconfig error: No writable cache directories`
warnings the container emits are noise: the cache cannot be written because
the shim runs as a non-root user with no writable `HOME`, and rebuilding it
each run costs milliseconds and changes nothing.

Text geometry is therefore no less reproducible than any other geometry —
3MF bytes still vary run to run per upstream #4931, but the measured geometry
does not.

### The real hazard: a missing font falls back silently
...
perfectly valid mesh. `--hardwarnings` does not catch it. A typo, or a font
removed by a pin bump, produces a part whose text is silently in the wrong
face — and since the result is a valid manifold mesh with plausible
dimensions, every existing gate passes it.

Font selection itself works; the five available families each produce
distinctly different geometry. The failure is confined to *unavailable*
names.

### Rules that follow

1. **Name the font explicitly. Never rely on the default.** The default is
   currently Liberation Sans — identical geometry confirms it — but that is
   an observation about this digest, not a contract. A pin bump could change
   it and nothing would say so.

2. **Use only families the image ships**, i.e. the table above. Anything else
   silently becomes Liberation Sans.

3. **CI must verify the font exists**, because nothing else will. The check
   does not need a font-enumeration API, which is fortunate since `fc-list`
   cannot see the bundled set: render the named font and a deliberately bogus
   name, and compare. Identical geometry means the named font is not
   installed and fell back.

   That is the same measure-don't-trust approach the rest of this design uses,
   and it tests what OpenSCAD actually did rather than what a font database
   claims is available.
...
With rule 3 in place, `text()` is safe to use, and the objection that kept it
out of `jig-drawer-pull-96mm-coupon` — where bores are indexed with notches
rather than printed numbers — no longer holds. Whether to revisit that part is
a separate decision: notches are legible after a coat of paint and do not care
about layer height, so the workaround has merits of its own.
...
unrelated to any of this anyway: committed thumbnails
(`thumbnails/<part>/<variant>.png`, generated by `make thumbnails`) reviewed
via GitHub's image diff in a PR — not a live in-editor preview. So dropping the extension isn't a gap
